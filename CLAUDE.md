...
└── projects/{주제-kebab-case}/
    ...
    │   └── draft.md
    └── output/                       # 완성본
        ├── 01_완성본.md              # 13항목 전체 (기획서 + 대본 + 제목/썸네일 + 편집 노트)
        └── 02_대본.txt               # TTS용 순수 나레이션
```
...
## 파일 인코딩
모든 산출물은 UTF-8. `02_대본.txt`는 나레이션 외 텍스트(제목·장면번호·연출지시·괄호·특수문자)를 넣지 않는다.
